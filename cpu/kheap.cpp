...
uint32_t kmalloc_int(size_t size, int align, uint32_t *phys_addr)
{
    if (kheap != 0)
    {
        void *addr = alloc_heap(size, (uint8_t)align, kheap);
        if (phys_addr != 0)
        {
...
uint32_t kmalloc_p(size_t size,uint32_t *phys_addr) // returns a physical address
{
    return kmalloc_int(size, 0, phys_addr);
}


uint32_t kmalloc_ap(size_t size, uint32_t *phys_addr) // page aligned and physical address
{
    ...
    while(iterator < heap->index.size){
        ...
        if(page_align > 0){
            ...
            if((location+sizeof(header_t) & 0xFFFFF000) != 0)
                offset = 0x1000 /*page size*/ - (location+sizeof(header_t))%1000;
...
heap_t *create_heap(uint32_t start,uint32_t end_addr, uint32_t max, uint8_t supervisor, uint8_t readonly){
    ...
    //start and end address need be page aligned
...
void free_heap(void *p, heap_t *heap){
    ...
    footer_t *footer = (footer_t*)((uint32_t)header + 
